...
window.getSuggestions = () => { 
    ...
    for(let u in window.allUsersData) { 
        ...
        // الشرط الصارم: يجب أن يكون هناك أصدقاء مشتركون أو من نفس الموقع الجغرافي
        if(mc > 0 || isl) {
            sg.push({name:u, data:d, mutualCount:mc, isSameLocation:isl}); 
        }
    } 
    // ترتيب: الأولوية للمشتركين ثم المقربين
    sg.sort((a,b) => { 
...
window.communities = JSON.parse(
    localStorage.getItem("communities")
) || [];

window.currentCommunity = null;
window.communityImage = "";
...
window.previewCommunityImage = function(event){
    ...
    reader.onload = function(e){
        ...
        window.communityImage = e.target.result;

        const img = document.getElementById(
            "communityPreviewImage"
        );

        img.src = window.communityImage;
...
window.createCommunity = function(){
    ...
    window.communities.unshift(community);

    localStorage.setItem(
        "communities",
        JSON.stringify(window.communities)
    );

    window.renderCommunities();

    window.closeModal("communityCreatorModal");
        ...
        container.innerHTML += `
            ...
            <div class="community-card">
                ...
                <div class="community-card-body">
                    ...
                    <button
                        onclick="window.openCommunity(${community.id})"
                        class="community-enter-btn"
                    >
                        دخول المجتمع
                    </button>
...
window.openCommunity = function(id){

    const community = window.communities.find(
        c => c.id == id
    );

    if(!community) return;

    alert("تم فتح المجتمع: " + community.name);
