...
function renderFeed() {
    ...
    // تم إيقاف عرض المنشورات الإخبارية المسجلة في الفيد بناءً على طلب المستخدم - القنوات نفسها تبقى متاحة للمتابعة فقط
...
function renderProfilePosts(u) { 
    ...
    if (isNewsBot) { $('profilePostsFeed').innerHTML = '<p style="text-align:center;color:#666;font-size:13px;">تم إيقاف عرض الأخبار المسجلة لهذه القناة.</p>'; $('profilePhotosGrid').innerHTML = `<a href="#/@${u}"><img src="${pp}" style="cursor:pointer;"></a>`; return; }
