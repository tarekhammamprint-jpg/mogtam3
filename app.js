...
function renderProfilePosts(u) { let pp = window.allUsersData[u]?.profilePic || dA; $('profilePostsFeed').innerHTML = '<div style="text-align:center;padding:20px;color:var(--primary);"><i class="fas fa-spinner fa-spin fa-2x"></i><br>جاري جلب المنشورات...</div>'; get(query(ref(db, 'posts'), orderByChild('author'), equalTo(u))).then(s => { let h = '', ph = ''; ph += `<a href="#/@${u}"><img src="${pp}" style="cursor:pointer;"></a>`; if(s.exists()) { let userPosts = []; s.forEach(c => { let p = c.val(); p.id = c.key; userPosts.push(p); window.postCache[p.id] = p; }); userPosts.sort((a,b) => b.timestamp - a.timestamp); userPosts.forEach(p => { if(!p.isReel) { let lc = p.likes ? Object.keys(p.likes).length : 0, it = lc >= 10; h += createPostHTML(p, 'profile', it, false); if(p.image) ph += `<a href="#/post/${p.id}"><img src="${p.image}" style="cursor:pointer;"></a>`; if(p.video) ph += `<a href="#/post/${p.id}"><video src="${p.video}" style="cursor:pointer;"></video></a>`; } }); } $('profilePostsFeed').innerHTML = h || '<p style="text-align:center;color:#666;font-size:13px;">لا مقالات.</p>'; $('profilePhotosGrid').innerHTML = ph; document.querySelectorAll('#profilePostsFeed video').forEach(v => window.videoObserver.observe(v)); }).catch(() => { let h2=''; Object.values(window.postCache||{}).filter(p=>p.author===u&&!p.isReel).forEach(p=>{ let lc=p.likes?Object.keys(p.likes).length:0; h2+=createPostHTML(p,'profile',lc>=10,false); }); $('profilePostsFeed').innerHTML=h2||'<p style="text-align:center;color:#666;font-size:13px;">لا مقالات.</p>'; }); let rh = '';
    let allForReels = [...(window.allReels||[]), ...Object.values(window.postCache||{})];
    let seenR = new Set();
    let userReels = allForReels.filter(p => { if(p.author===u && p.video && !seenR.has(p.id)){seenR.add(p.id);return true;} return false; });
    userReels.forEach(r => {
        let gIdx = window.allReels.findIndex(x => x.id === r.id);
        if(gIdx === -1){ window.allReels.push(r); gIdx = window.allReels.length-1; }
        let vc = r.views ? Object.keys(r.views).length : 0;
        rh += `<div class="reel-thumb" style="width:100%;height:180px;" onclick="window.openReelsViewer(${gIdx})"><video src="${r.video}" autoplay loop muted playsinline preload="auto" style="pointer-events:none;background:#1e293b;object-fit:cover;"></video><span class="r-views"><i class="fas fa-play"></i> ${vc}</span></div>`;
    });
    ...
    onValue(ref(db, `users/${u}/banned`), (snap) => {
        if (snap.val() === true) {
            get(ref(db, `users/${u}`)).then(s => {
                if (!s.exists()) return;
                let ud = s.val(); let now = Date.now();
                let perm = !ud.banUntil || ud.banUntil === 0;
                let expired = !perm && ud.banUntil < now;
                if (!expired) { set(ref(db, `users/${u}/online`), false); localStorage.removeItem('savedUser'); window.showBanScreen(ud); }
            });
        }
    });
...
window.generateReelsWidgetHTML = () => {
    ...
    window.allReels.slice(0, 6).forEach((r) => {
...
window.openReelsLogic = (startIdx) => {
    ...
    area.innerHTML = '';
    let reels = (window.allReels && window.allReels.length > 0) ? window.allReels : (window.allPosts || []).filter(p => p.video && !p.isNewsBot);
    if (reels.length === 0) { area.innerHTML = '<p style="text-align:center;color:#fff;padding:40px;">لا يوجد ريلز حالياً.</p>'; modal.classList.add('show'); document.body.style.overflow = 'hidden'; return; }
    reels.forEach((r) => {
        let ap = (window.allUsersData[r.author]?.profilePic) || dA;
        let dn = window.getDisplayName(r.author);
        let vc = r.views ? Object.keys(r.views).length : 0;
    ...
    setTimeout(() => {
        ...
        let firstVid = items[idx]?.querySelector('video');
