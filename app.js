...
// -- متغيرات منع التكرار والمجتمعات --
...
function handleRouting() {
    ...
    document.querySelectorAll('.modal').forEach(m => {
        if(m.id !== 'interestsModal' && m.id !== 'communitiesModal' && m.id !== 'communityViewModal') m.classList.remove('show');
    });
...
window.closeModal = (id) => { 
    if(window.history.length > 2 && !['communitiesModal','communityViewModal'].includes(id)) { window.history.back(); } 
    else { window.location.hash = ''; let m = $(id); if(m) m.classList.remove('show'); document.body.style.overflow = 'auto'; }
};
...
// ======================== نظام المجتمعات (Groups & Video/Audio) ========================
...
window.createCommunity = async () => {
    ...
    let btn = $('createCommunityBtn'), ot = btn.innerHTML; 
...
window.publishCommunityPost = () => {
    ...
    if(!txt || !window.currentCommunityId) return;
    let btn = $('publishCommBtn'); let ot = btn.innerHTML;
    ...
    push(ref(db, `communityPosts/${window.currentCommunityId}`), d).then(() => {
...
function renderCommunityFeed(commId) {
    ...
    window.currentCommListener = onValue(query(ref(db, `communityPosts/${commId}`), limitToLast(30)), s => {
        ...
        if(s.exists()){
            let posts = [];
            s.forEach(c => { posts.push({...c.val(), id: c.key}); });
            posts.sort((a,b) => b.timestamp - a.timestamp);
            
            posts.forEach(p => {
                let dt = new Date(p.timestamp).toLocaleString('ar-EG'); 
...
function listenToUsers(){ 
    onValue(ref(db,'users'), s => { 
        if(s.exists()){ 
            ...
            if(window.isInitialLoad){ listenToPosts(); }
...
function listenToCommunities() {
    onValue(ref(db, 'communities'), s => {
        ...
        if($('communitiesModal') && $('communitiesModal').classList.contains('show')) {
            renderCommunitiesList();
        }
    });
}
...
window.addEventListener('scroll', () => { 
    if((window.innerHeight+window.scrollY) >= document.body.offsetHeight-800){ 
        if(window.feedLim < window.allPosts.length){ window.feedLim += 5; renderFeed(); } 
    } 
});
...
function renderFeed() {
    ...
    window.allPosts.forEach(p => { 
        ...
        if(iN){ if(im || it) vp.push({p:p, it:it}); } 
        else { if(im || ifR) reg.push({p:p, it:it}); else if(it) tr.push({p:p, it:true}); } 
    });
...
window.renderPostModalLogic = (p) => {
    ...
    let pf = $('postModalFooter'); if(pf) pf.style.display = window.currentUser ? 'flex' : 'none';
    let closeBtn = $('postModalCloseBtn'); if(closeBtn) closeBtn.style.display = 'flex';
...
window.openProfileLogic = (u) => {
    ...
    if(!d) {
        get(ref(db, `users/${u}`)).then(s => {
            if(s.exists()){ window.allUsersData[u] = s.val(); renderProfileData(u, s.val()); } else { alert("الحساب غير موجود."); window.goHome(); }
        });
    } else { renderProfileData(u, d); }
};
...
function renderProfileData(u, d) {
    ...
    if(d.interests && d.interests.length > 0) { intArea.style.display = 'flex'; intArea.innerHTML = d.interests.map(i => `<span style="background:#eef2ff; color:var(--primary); padding:4px 10px; border-radius:12px; font-size:12px; font-weight:700;">${i}</span>`).join(''); } else { intArea.style.display = 'none'; }
    ...
    if(!window.currentUser) { $('coverEditBtn').style.display = 'none'; ac.innerHTML = `<button class="btn-primary" onclick="window.showRegisterModal()"><i class="fas fa-user-plus"></i> تسجيل الدخول للتفاعل</button>`; } 
...
function renderProfilePosts(u) { 
    ...
    get(query(ref(db, 'posts'), orderByChild('author'), equalTo(u))).then(s => { 
        let h = '', ph = ''; ph += `<a href="#/@${u}"><img src="${pp}" style="cursor:pointer;"></a>`; 
        if(s.exists()) { 
    ...
    if(userReels.length > 0) { userReels.forEach(r => { let globalIdx = window.allReels.findIndex(x => x.id === r.id); let vc = r.views ? Object.keys(r.views).length : 0; rh += `<div class="reel-thumb" style="width:100%; height:180px;" onclick="window.openReelsViewer(${globalIdx})"><video src="${r.video}" autoplay loop muted playsinline preload="auto" poster="${reelPoster}" style="pointer-events:none; background:#1e293b; object-fit:cover;"></video><span class="r-views"><i class="fas fa-play"></i> ${vc}</span></div>`; }); }
...
window.unfriend = (t) => { if(!window.currentUser) return; if(confirm(`حذف الصداقة؟`)) { let up = {}; up[`friends/${window.currentUser}/${t}`] = null; up[`friends/${t}/${window.currentUser}`] = null; update(ref(db), up).then(() => window.openProfile(t)); } };
...
window.getSuggestions = () => { 
    ...
    for(let u in window.allUsersData) { 
        ...
        if(mc > 0 || isl) { sg.push({name:u, data:d, mutualCount:mc, isSameLocation:isl}); }
    } 
    sg.sort((a,b) => { 
